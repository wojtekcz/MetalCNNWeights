...
class Inception3Net{
    ...
    let sftid = MPSImageDescriptor(channelFormat: textureFormat, width: 1  , height: 1  , featureChannels: 120)
    ...
    init(withCommandQueue inputCommandQueue : MTLCommandQueue){
        ...
        fc0 = SlimMPSCNNFullyConnected(kernelWidth: 1,
                                       kernelHeight: 1,
                                       inputFeatureChannels: 2048, // the Inception V3 model's top layer receives an input a 2048-dimensional vector for each image. A softmax layer is trained on top of the vector. For the 1000 label classification, # of trained parameters from the top layer to the result is 1000 (biases#) + 2048*1000
                                       outputFeatureChannels: 120, // 1008,
                                       neuronFilter: nil,
                                       device: device,
